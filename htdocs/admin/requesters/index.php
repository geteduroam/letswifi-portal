...
if ( 'POST' === $_SERVER['REQUEST_METHOD'] ) {
	...
	if ( \array_key_exists( 'revoke_requester', $_POST ) && \is_string( $_POST['revoke_requester'] ) ) {
		$credentialAdmin->revokeRequester( $_POST['revoke_requester'], $validOn, $realms );
	}
...
$app->render( [
	...
	RequesterAggregate::class => static fn ( RequesterAggregate $ra ) => [
		...
		'credential_unrevoked_href' => '../credentials/?' . \http_build_query( ['unrevoked' => 'on', 'requester' => $ra->requester->name, 'realms' => $ra->requester->realm] ),
